...
#RUN echo -e "[ODBC Drivers]\nMongoDB Atlas SQL ODBC Driver = Installed\n\n[MongoDB Atlas SQL ODBC Driver]\nDriver=/usr/local/lib/mongoodbc/bin/libatsql.so" >> /etc/odbcinst.ini

#RUN echo -e "[ODBC Data Sources]\nMongoDB_Atlas_SQL = \"MongoDB Atlas SQL ODBC Driver\"\n\n[MongoDB_Atlas_SQL]\nPassword = $HARPER__MONGO__PASS\nDriver = /usr/local/lib/mongoodbc/bin/libatsql.so\nDatabase = $HARPER__MONGO__DB\nUser = $HARPER__MONGO__USER\nUri = $HARPER__MONGO__URI\nUnicodeTranslationOption = utf16" >> /etc/odbc.ini
