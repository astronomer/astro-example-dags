{% if is_modified %}
DROP MATERIALIZED VIEW IF EXISTS {{ schema }}.rep__currently_return_required_orders CASCADE;
{% endif %}

CREATE MATERIALIZED VIEW IF NOT EXISTS {{ schema }}.rep__currently_return_required_orders AS
    SELECT
            o.order_name,
            o.order_status,
            o.order_type,
    ...
    WHERE o.order_status='return_required' and link_order_child=0
    ORDER BY createdAt DESC
WITH NO DATA;

{% if is_modified %}
CREATE UNIQUE INDEX IF NOT EXISTS rep__currently_return_required_orders_idx ON {{ schema }}.rep__currently_return_required_orders (id);
{% endif %}

REFRESH MATERIALIZED VIEW {{ schema }}.rep__currently_return_required_orders;
