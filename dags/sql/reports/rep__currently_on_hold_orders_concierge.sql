{% if is_modified %}
DROP MATERIALIZED VIEW IF EXISTS {{ schema }}.rep__currently_on_hold_orders_concierge CASCADE;
{% endif %}

CREATE MATERIALIZED VIEW IF NOT EXISTS {{ schema }}.rep__currently_on_hold_orders_concierge AS
    SELECT
            o.order_name,
            o.order_status,
            o.order_type,
    ...
    WHERE o.order_status='hold' AND o.is_harper_try=False and link_order_child=0
    ORDER BY createdAt DESC
WITH NO DATA;

{% if is_modified %}
CREATE UNIQUE INDEX IF NOT EXISTS rep__currently_on_hold_orders_concierge_idx ON {{ schema }}.rep__currently_on_hold_orders_concierge (id);
{% endif %}
